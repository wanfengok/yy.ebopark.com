...
class UserController extends BaseController
{
    ...
    public function register(){
        ...
        $uid =$ucenterMemberModel ->register($username, $nick, think_ucenter_md5($password, UC_AUTH_KEY));
        if(0<$uid){
            //注册成功
    ...
    public function index()
    {
        ...
        foreach($list as $key=>&$value){
            $uid=$value["uid"];
                $res=M()->query("select title from oms_auth_group where id IN(select group_id as id from oms_auth_group_access where uid=$uid) and status=1");
                $roleArr=[];
                foreach($res as $key1=>$value1){
                    array_push($roleArr,$value1["title"]);
                }
                $value["role"]=implode(', ',$roleArr);
    ...
    public function initPass()
    {
        ...
        $data = $ucModel->create(array('password' => think_ucenter_md5('123456',UC_AUTH_KEY)));
    ...
    public function expandinfo_select($page = 1, $r = 20)
    {
        ...
        $field_group_ids = array_column_i($field_group, 'id');
        ...
        $fields_list = array_combine(array_column_i($fields_list, 'field_name'), $fields_list);
    ...
    public function expandinfo_details($uid = 0)
    {
        if (IS_POST) {
            ...
            if (count($already_role)) {
                $already_role = array_column_i($already_role, 'role_id');
            }
            $roleModel = D('Role');
            $role_key = array();
    ...
    public function editFieldSetting($id = 0, $profile_group_id = 0, $field_name = '', $child_form_type = 0, $visiable = 0, $required = 0, $form_type = 0, $form_default_value = '', $validation = 0, $input_tips = '')
    {
        if (IS_POST) {
            ...
            if ($id != 0) {
                ...
                $already_role_id = array_column_i($already_role_id, 'role_id');
    ...
    public function submitNickname()
    {
        ...
        //密码验证
        $User = new UserApi();
        $uid = $User->login(session("user_auth")["username"], $password, 4);
        ($uid <= 0) && $this->error(L('_INCORRECT_PASSWORD_'));
